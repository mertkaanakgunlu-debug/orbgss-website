...
**Canonical version:** `v1.0.0-rc3-web-005a-hero-visual-revision` — WEB-005A `REVIEW_READY` at final production (hero media, drape states and hero integration replaced; rest of the release candidate unchanged)
**Date:** 2026-09-19
**Stage:** WEB-005A / MER-107 is **`REVIEW_READY` at final production** (2026-09-19): after three low-cost preview gates Product moved the gate-3 direction to production quality in chat (preserve its storytelling; strong right-side analytical payoff; clean left copy-safe area; prepared 4K assets; no colour bar, legend, card or layer label in the hero). Delivered on `feat/web-005a-hero-visual-fidelity`: the accepted choreography promoted into `hero_production_kizildere` (the gate scene is now an alias of it), a 276-frame production motion render encoded to WebM / MP4 / poster with **no analytical pixel in any lossy encode**, four **lossless** page-composited drape states (Terrain → THM-01 → ALT-01 → Priority) rendered from the prepared 4K display textures on governed-DEM relief, and the homepage hero integration (full-frame states sharing the poster's fit rule; no stage, legend, card or label; desktop ≥ 981 px only). Evidence and review pointer: `tasks/WEB-005A_FINAL_PRODUCTION_EVIDENCE.md`, `hero/evidence/web005a_final/`. Gates: site validator PASS, hero validator 418 / 0, production audit 59 / 59, negative tests 47 / 47. Awaiting Product review. *History of the revision follows.* Previously, WEB-005A was in **`REVISION_REQUIRED`** after Product/CTO human visual review of `37152222` (`docs/web-005-polish-authority@473b48a00bfe85d3dbe1cfe7219105c2b5ec7574:tasks/WEB-005A_R3_REVIEW_37152222.md`): the camera moved before the scan completed (A-HERO-15), the platform sat under the hero copy on the real 1440 × 900 page, and the analytical reveal was a planar page overlay rather than DEM relief (A-HERO-18/19). The review requires a low-cost **preview gate** before any further long production render. The first preview (`tasks/WEB-005A_R3_PREVIEW_GATE.md`, `cd2018e`, evidence `hero/evidence/web005a_r3_preview/`) was reviewed `PREVIEW_GATE_REVISION_REQUIRED / CONTINUATION_AUTHORIZED` (`docs/web-005-polish-authority@c7c6cb10e305c9de63d805c414b332440fd87ddb:tasks/WEB-005A_R3_PREVIEW_GATE_PRODUCT_DECISION.md`): the four primary lines must end on the true governed 36 km corners, the presentation reticle must be separate from true ground geometry, the platform exit must stay inside the 22 % fly-by bound, the final payoff must be protected from the left readability shade, and the shipping envelope is about 12 s of motion plus a 2.4–3.2 s analytical sequence. Product then added six clarifications (settle → aim → draw-on sequencing, lines that propagate, thinner line language, a cyan / teal scan palette, no hero colour bar or information card, brisker timing). The corrected second preview (`tasks/WEB-005A_R3_PREVIEW_GATE_2.md`, `b6cd5fb`, evidence `hero/evidence/web005a_r3_preview2/`) was reviewed by Product/CTO as motion and returned `REVISION_REQUIRED`: four lines onto the true 36 km footprint read as "beams go toward the middle, then the AOI appears". Product then overrode `c7c6cb1` §2 for the *visible* acquisition — four lines to the four corners of a presentation reticle centred on the true AOI, legible before any line, tightening onto the true AOI afterwards, with everything analytical still on the true footprint — and `docs/web-005-polish-authority@b9579efbfa0c13342ac24a40e825909ac2845957` resolved gate 2's open points (no caption crossing; masked analytical ground shows Terrain; no hero legend, card or label). The **third preview is now submitted**: `tasks/WEB-005A_R3_PREVIEW_GATE_3.md` (same scene `hero_r3_preview_gate`, evidence `hero/evidence/web005a_r3_preview3/`). It changes no site file, no shipped media and not the production scene; WEB-005A returns to `REVIEW_READY` only after the gate is passed, the conforming production output is rendered and the integrated homepage is re-submitted. *The remainder of this paragraph describes the reviewed R3 checkpoint, which is what the branch still ships:* WEB-005A (hero visual revision, Product R2 visual lock as re-reviewed) was published `REVIEW_READY` at **R3** on `feat/web-005a-hero-visual-fidelity`. The R2 package (`a6897828d6e35595ea5a9a39a1e812b0fcbbfb35`) was technically green and did **not** pass the human visual gate; like the `ffa2f2944ca5992afb9e9891b42745a9bd1105ad` checkpoint before it, it is retained as before-evidence and is not accepted. R3 re-choreographs the same hero: a satellite that comes round the limb and settles for the whole acquisition beat, sensing lines that release while it is still in frame, one continuous dive onto the accepted 36 km analysis AOI with its own lock event, 30 m ground structure under the hold, and a page-layer evidence sequence (terrain → THM-01 → ALT-01 → priority) registered inside the analysis frame. Evidence: `tasks/WEB-005A_HERO_FIDELITY_EVIDENCE.md`. **Not merged to `main`. Nothing deployed. No DNS touched. WEB-005B and WEB-006 have not begun.**
...
**Tracking:** MER-90 (WEB-002 accepted); MER-91 (WEB-003 accepted); MER-92 (WEB-004 accepted); MER-102 / GEO-WEB-002 (Science-accepted); MER-101 / WEB-HERO-001D (terminally accepted); MER-93 / WEB-005 (terminally accepted); **MER-107 / WEB-005A (REVIEW_READY at final polish: relief rise + startup poster)**; MER-96 / GEO-WEB-001 resolved
