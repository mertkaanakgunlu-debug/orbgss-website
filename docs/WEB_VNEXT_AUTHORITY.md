...
- **WEB-004** — responsive, accessibility, performance and hosted preview acceptance — **IMPLEMENTED / REVIEW_READY** on `feat/web-004-preview-hardening`, from accepted `main@f4d77b4144ddff70c309986e6a45b163f62cdcd4` under authority publication `3a4d8e006add2725d45b02071bda654be5bd09f5`. Evidence: `docs/WEB-004_HARDENING_EVIDENCE.md`. Product decisions are resolved on MER-92 and recorded in `tasks/WEB-004_PREVIEW_HARDENING.md`: homepage LCP 3.2 s under the local synthetic profile and the score-legend `image-aspect-ratio` finding are accepted bounded intentional deviations (accepted scientific proof derivatives must not be re-encoded to chase 2.5 s; the scientific legend must not be re-proportioned); the WEB-005 hero media budget is accepted (WebM ≤ 3.0 MiB, MP4 ≤ 4.5 MiB, poster ≤ 180 KiB, mobile/reduced-data an intentional static/poster fallback with no dual-video download); `HOSTED_PREVIEW_NOT_RUN — permission unavailable` is accepted under the task; `CONTACT_RELEASE_GATE` remains open for pre-WEB-006 launch verification. No Product/Science semantics changed. Product performs terminal acceptance/merge.
